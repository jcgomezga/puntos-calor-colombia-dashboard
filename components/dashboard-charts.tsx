...
function EmptyChart({ label }: { label: string }) {
  return <div className="chart-empty" role="status" aria-label={label}>No hay detecciones para los filtros seleccionados.</div>;
}

export function RankingChart({ data, label }: { data: RankingDatum[]; label: string }) {
  if (!data.length) return <EmptyChart label={label} />;
  const summary = data.map((item) => `${item.name}: ${numberFormat.format(item.value)} detecciones`).join("; ");
...
export function TrendChart({ data, label }: { data: TrendDatum[]; label: string }) {
  if (!data.length || data.every((item) => item.value === 0)) return <EmptyChart label={label} />;
  const summary = data.map((item) => `${item.label}: ${numberFormat.format(item.value)} detecciones`).join("; ");

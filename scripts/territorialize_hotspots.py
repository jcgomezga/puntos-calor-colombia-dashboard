...
def territorialize(data_dir: Path, records: list[PolygonRecord]) -> tuple[list[dict[str, str]], Counter, list[dict[str, object]]]:
...
def run(args: argparse.Namespace) -> int:
    ...
    if not (public_dir / "departments.geojson").exists() or args.refresh_boundaries:
        ...
        atomic_write(public_dir / "departments.geojson", json_bytes(departments))
    ...
    rows, statuses, outputs = territorialize(data_dir, records)
    if not rows:
        raise RuntimeError("no existen hotspots normalizados")
    atomic_write(public_dir / "municipalities.geojson", json_bytes(municipality_display(municipal)))
    ...
    files = {}
    for name in ("dashboard.json", "departments.geojson", "municipalities.geojson"):

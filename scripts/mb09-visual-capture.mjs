...
const ARTIFACT_DIR = resolve(process.cwd(), "artifacts/mb09-visual");
const PORT = Number(process.env.MB09_VISUAL_PORT ?? 4177);
const CDP_PORT = Number(process.env.MB09_VISUAL_CDP_PORT ?? 9227);
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (range) {
        ...
        if (!Number.isFinite(start) || start < 0 || start > end || start >= metadata.size) { response.writeHead(416, { "content-range": `bytes */${metadata.size}` }); response.end(); return; }
...
function findChrome() {
  for (const command of [process.env.CHROME_PATH, "google-chrome", "google-chrome-stable", "chromium", "chromium-browser"].filter(Boolean)) {
...
async function connectCdp(url) {
  ...
  socket.addEventListener("message", (event) => {
    ...
    pending.delete(message.id); if (message.error) waiter.reject(new Error(message.error.message)); else waiter.resolve(message.result);
  });
...
async function evaluate(send, expression) {
  ...
  if (result.exceptionDetails) throw new Error(result.exceptionDetails.exception?.description ?? result.exceptionDetails.text ?? "Error CDP");
...
const probe = `(() => {
  ...
  return {
    ...
    geovisor,
    layerControl,
    layerControlOcclusionRatio: overlapRatio(geovisor, layerControl),
    collapsed: document.querySelector(".layer-control")?.classList.contains("collapsed") ?? false,
    open: document.querySelector(".layer-control")?.classList.contains("open") ?? false,
    canvasCount: document.querySelectorAll(".geovisor-map .maplibregl-canvas").length,
    error: document.querySelector(".geovisor-error")?.textContent?.trim() ?? "",
    coverageStatus: document.querySelector(".coverage-status")?.textContent?.trim() ?? "",
  };
})()`;

let server, chrome, cdp, profileDir;
const result = { desktop: null, context: null, mobileCollapsed: null, mobileOpen: null };
try {
  await mkdir(ARTIFACT_DIR, { recursive: true });
  server = await startStaticServer();
  profileDir = await mkdtemp(`${tmpdir()}/mb09-visual-`);
  ...
  if (!await waitFor(cdp.send, `Boolean(document.querySelector(".geovisor-map .maplibregl-canvas"))`, 25_000)) throw new Error("MapLibre no creó canvas.");
  await focusMap(cdp.send); await delay(4_000);
  result.desktop = await evaluate(cdp.send, probe);
  await screenshot(cdp.send, "01-geovisor-desktop.png");

  await evaluate(cdp.send, `(() => {
    const wanted = ["Áreas protegidas RUNAP", "Títulos mineros ANM", "Proyectos ANLA", "Áreas asignadas ANH"];
    for (const label of document.querySelectorAll(".layer-control-body label")) {
  ...
  await delay(2_500);
  result.context = await evaluate(cdp.send, probe);
  await screenshot(cdp.send, "02-geovisor-contexto.png");

  await cdp.send("Emulation.setDeviceMetricsOverride", { width: 390, height: 844, deviceScaleFactor: 1, mobile: true });
  await waitFor(cdp.send, `document.querySelector(".layer-control")?.classList.contains("collapsed") === true`, 8_000);
  await focusMap(cdp.send); await delay(1_000);
  result.mobileCollapsed = await evaluate(cdp.send, probe);
  await screenshot(cdp.send, "03-geovisor-mobile-colapsado.png");

  await evaluate(cdp.send, `document.querySelector(".layer-control-toggle")?.click()`);
  await waitFor(cdp.send, `document.querySelector(".layer-control")?.classList.contains("open") === true`, 5_000);
  await delay(500);
  result.mobileOpen = await evaluate(cdp.send, probe);
  await screenshot(cdp.send, "04-geovisor-mobile-panel.png");

  await writeFile(resolve(ARTIFACT_DIR, "metrics.json"), JSON.stringify(result, null, 2));

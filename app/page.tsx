...
export default function Home() {
  ...
  return <main className="dashboard-shell">
    ...
    <section className="filterbar" aria-label="Filtros territoriales">
      ...
      <label><span>Departamento</span><div className="select-wrap"><select value={departmentCode} onChange={(e) => { setDepartmentCode(e.target.value); setMunicipalityCode("00000"); }}><option value="00">Todos los departamentos</option>{dashboard.departments.map((item) => <option key={item.code} value={item.code}>{item.name}</option>)}</select><ChevronDown size={16} /></div></label>
      <label><span>Municipio</span><div className="select-wrap"><select value={municipalityCode} disabled={departmentCode === "00"} onChange={(e) => setMunicipalityCode(e.target.value)}><option value="00000">Todos los municipios</option>{municipalityOptions.map((item) => <option key={item.code} value={item.code}>{item.name}</option>)}</select><ChevronDown size={16} /></div></label>
      ...
      <label><span>Situación ANLA</span><div className="select-wrap"><select value={anlaLegalStatus} disabled={!dashboard.metadata.anlaProjects} onChange={(e) => setAnlaLegalStatus(e.target.value as AnlaLegalStatus)}><option value="all">Evaluación y licenciados</option><option value="evaluation">En evaluación</option><option value="licensed">Licenciados</option></select><ChevronDown size={16} /></div></label>
